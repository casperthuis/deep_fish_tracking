...
def main():
    ...
    parser.add_argument('--resolution', 
        ...
        nargs='+',
        type=int,
        help='The resolution of the vid frames, please provide a tuple.')
    ...
    for i, img in enumerate(imgs):
        if isinstance(img, str):
            ...
            img = mmcv.imread(img)
        if args.resolution:
            img = mmcv.image.imresize(img, tuple(args.resolution))    
        if draw_frame:
            if args.gt_bbox_file is not None:
                bboxes = mmcv.list_from_file(args.gt_bbox_file)
                init_bbox = list(map(float, bboxes[0].split(',')))
            else:
                img = cv2.putText(img, "q to quit, s to skip, a to annotated", (50, 50), cv2.FONT_HERSHEY_SIMPLEX,  1, (0,0,255), 4) 
        ...
        result = inference_sot(model, img, init_bbox, frame_id=j)
        j += 1
        if args.output is not None:
